...
import { ArrowRight, CheckCircle2, FileText, Shield, ArrowLeft, Loader2, XCircle, Lock, AlertCircle } from "lucide-react"
...
export default function RehabForm270Page() {
  ...
  useEffect(() => {
    ...
    const loadCaseData = async () => {
      try {
        ...
        // If form 270 was already submitted, redirect to dashboard immediately
        if (c.status === 'form_270_submitted') {
  ...
  const validate = (): Record<string, string> => {
    ...
    if (form.repeatEmail.trim() !== form.email.trim())
      r("repeatEmail", isRTL ? 'כתובות הדוא"ל אינן זהות' : "Email addresses do not match")
          ...
          {Object.keys(validationErrors).length > 0 && (
            <div className="p-4 bg-red-50 rounded-lg border border-red-200 text-sm text-red-800 space-y-1">
              <p className="font-semibold">
                {isRTL ? "יש לתקן את השדות הבאים לפני הגשה:" : "Please fix the following before submitting:"}
              </p>
